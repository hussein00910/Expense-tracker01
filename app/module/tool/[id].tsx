...
export default function ToolScreen() {
  ...
  if (!tool || !module) {
    return (<View style={{ flex: 1, backgroundColor: "#0a0a0a", alignItems: "center", justifyContent: "center" }}><Text style={{ color: "#ff3333" }}>Tool not found</Text></View>);
  }

  function updateParam(name: string, value: string) { setParams((prev) => ({ ...prev, [name]: value })); }

  function validateParams(): boolean {
    for (const p of tool!.params) {
      if (p.required && !params[p.name]?.trim()) { Alert.alert("Missing Parameter", `"${p.label}" is required.`); return false; }
    }
    return true;
  }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#0a0a0a" }} edges={["bottom"]}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView ref={scrollRef} contentContainerStyle={{ padding: 16 }} keyboardShouldPersistTaps="handled">
          ...
          {tool.params.map((param) => (<ParamInput key={param.name} param={param} value={params[param.name] ?? ""} onChange={(v) => updateParam(param.name, v)} />))}
          <TouchableOpacity onPress={handleExecute} disabled={loading}
          ...
          >
            {loading ? (
              <><ActivityIndicator color="#00ff41" size="small" /><Text style={{ color: "#00ff41", fontSize: 15, fontWeight: "700" }}>Executing...</Text></>
            ) : (
              <><MaterialCommunityIcons name="play" size={18} color="#0a0a0a" /><Text style={{ color: "#0a0a0a", fontSize: 15, fontWeight: "700" }}>Execute</Text></>
            )}
          </TouchableOpacity>
          {result && (
            <><Text style={{ color: "#555555", fontSize: 11, letterSpacing: 2, marginBottom: 12 }}>OUTPUT</Text><CommandOutput result={result} /></>
          )}
        </ScrollView>
